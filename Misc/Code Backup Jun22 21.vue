...
<template>
  <div id="app">
    ...
    <span style="position: absolute; bottom: 10px" v-if="records.length > 0">
      ...
      <p style="margin-right: 0; display: inline" class="display" id="lyrictxt">
        {{ lyrictxt }}
      </p>
      <p
        style="margin-left: 0; display: inline; color: #999999"
        class="display"
        id="nplydtxt"
      >
        {{ nplydtxt }}
...
const INIT_MCFUNC =
  "";

// core/main.mcfunction
const MAIN_MCFUNC =
  "";

// core/track.mcfunction
const TRACK_MCFUNC =
  "";

// 此处的index对应游戏中的CustomModelData, 相当于下文for循环遍历records时的(Number(this.indexstart) + Number(rec))
function getPlayCmd(index, id, pns, base) {
  return (
    'execute if data block ~ ~ ~ RecordItem{id:"minecraft:music_disc_' +
    base +
    '",tag:{CustomModelData:' +
    index +
    "}} as @e[type=marker,limit=1] run function " +
    pns +
    ":custom/" +
    id +
    "\n"
...
function getGiveCmd(index, artist, title, base, color) {
  return (
    "give @p minecraft:music_disc_" +
    base +
    "{CustomModelData:" +
    index +
    ', display:{Lore:["{\\"text\\":\\"' +
    ...
    '\\",\\"color\\":\\"' +
    color +
    '\\",\\"bold\\":false,\\"italic\\":false}"]}, HideFlags:32}\n'
  );
}
function getDispLrcCmd(index, id, pns) {
  return (
    "execute as @e[type=marker,tag=disc,scores={curmusic=" +
    index +
    "}] run function " +
    pns +
    ":custom/" +
...
function getTitleCommand(index, tic, disp, txt1, cl1, txt2, cl2, txt3, cl3) {
  txt1 = txt1
    .replace(/[\"\']/g, "\\$&")
    .replace(/[\n\r\u0085\u2028\u2029]/g, ""); // Note that lyrics may also contain quotation marks, so we change 'em here...
  txt2 = txt2
    .replace(/[\"\']/g, "\\$&")
    .replace(/[\n\r\u0085\u2028\u2029]/g, "");
  //console.log(txt);
  return (
    "execute if score @s playtick matches " +
    tic +
    " run title @e[type=player,scores={lrc=" +
    index +
    "}] " +
    disp +
    ' "' +
    cl1 +
    (txt1 == EMPTY_SYMBOL ? " " : txt1) +
    cl2 +
    (txt1 == EMPTY_SYMBOL || txt2 == "" ? "" : txt2) +
    cl3 +
    (txt1 == EMPTY_SYMBOL ||
    txt3.replace(/ /g, "") == "" ||
    txt3 == EMPTY_SYMBOL
...
function genTitleCommand(index, id, pns, artist, title) {
  var finalres =
    getTitleCommand(index, 1, "subtitle", artist, "", "", "", "", "") + "\n";
  finalres +=
    getTitleCommand(index, 1, "title", title, "", "", "", "", "") + "\n";

  var curlrcln = "";
  var curtrsln = "";

  for (var tik = 0; tik < TICK_LIMIT; tik++) {
    if (trslnfortick[tik]) {
      if (trslnfortick[tik] == EMPTY_SYMBOL) curtrsln = "";
      else curtrsln = trslnfortick[tik];
    }
    if (lrclnfortick[tik]) {
      if (lrclnfortick[tik] == EMPTY_SYMBOL) curlrcln = "";
      else curlrcln = lrclnfortick[tik];
    }
    if (lyricfortick[tik]) {
      finalres += getTitleCommand(
        index,
        tik,
        "actionbar",
        lyricfortick[tik],
        "§b",
        curlrcln.substr(lyricfortick[tik].length),
        "§9",
        curtrsln,
        "§3"
      );
...
function genPlayCommand(index, id) {
  return (
    "playsound record." +
    id +
    " record @a\nscoreboard players add @s playtick 1\nscoreboard players set @s curmusic " +
    index +
    "\nscoreboard players set @e[type=player,distance=..9] lrc " +
    index
  );
}
function clearLrc() {
...
export default {
  ...
  methods: {
    ...
    getFile(event) {
      ...
      this.pagetitle = "𝕃𝕠𝕒𝕕𝕚𝕟𝕘 𝕃𝕪𝕣𝕚𝕔𝕤...ℙ𝕝𝕫 𝕎𝕒𝕚𝕥";
      setTimeout(() => {
        this.initLyrics();
    ...
    initLyrics() {
      ...
      this.pagetitle = "𝕮𝖗𝖊𝖊𝖕𝖊𝖗𝖘 𝖔𝖓 𝖁𝖊𝖓𝖚𝖘";
    },
    updateDisplay() {
      ...
      if (lrclnfortick[this.playtick]) {
        if (lrclnfortick[this.playtick] == EMPTY_SYMBOL) this.lrclntxt = "-";
        else this.lrclntxt = lrclnfortick[this.playtick];
      }

      if (lyricfortick[this.playtick]) {
        if (lyricfortick[this.playtick] == EMPTY_SYMBOL) {
          this.lyrictxt = "-";
          this.nplydtxt = "";
        } else {
          this.lyrictxt = lyricfortick[this.playtick];
          this.nplydtxt = this.lrclntxt.substr(
            lyricfortick[this.playtick].length
          );
        }
      }
    ...
    selectIcon(id) {
      console.log("Select Icon For " + id);
      this.editico = id;
      // Pop out the icon selector...
      var e = document.createEvent("MouseEvents");
      e.initEvent("click", true, true);
      this.$refs.icosel.dispatchEvent(e);
    },
    setIcon(event) {
      if (event.target.files.length == 0) return;
      const newico = event.target.files[0];
      const target = this.editico;
      const reader = new FileReader();
      const trec = this.records;
      reader.readAsDataURL(newico);
      reader.onload = function () {
        for (var rec in trec) {
          if (trec[rec].id == target) {
            trec[rec].icon = reader.result; //获取到base64格式图片
          }
        }
      };
    },
    generateResPack() {
      ...
      for (var rec in this.records) {
        baseModelObj.overrides.push({
          predicate: {
            custom_model_data: Number(this.indexstart) + Number(rec)
          },
          model: this.packnamespace + ":item/" + this.records[rec].id
        });
      }
      const fiblob2 = new Blob([JSON.stringify(baseModelObj, "", 4)], {
        type: "application/json"
      });
      mcmodelfolder.file("music_disc_" + this.discbase + ".json", fiblob2, {
        blob: true
      });
      this.pagetitle = "𝔾𝕖𝕥 𝕐𝕠𝕦𝕣 ℝ𝕖𝕤ℙ𝕒𝕔𝕜!";
      zip.generateAsync({ type: "blob" }).then(function (content) {
        saveAs(content, "DiscPlus Res Pack.zip");
      });
    },
    generateDatPack() {
      //this.pagetitle = "Generating DatPack...";
      var zip = new JSZip();
      ...
      mcfunctagfolder.file(
        "load.json",
        JSON.stringify({ values: [this.packnamespace + ":core/init"] }, "", 4)
      );
      mcfunctagfolder.file(
        "tick.json",
        JSON.stringify(
          {
            values: [
              this.packnamespace + ":core/main",
              this.packnamespace + ":display_lrc"
            ]
          },
          "",
          4
      ...
      var packcorefuncfolder = zip.folder(
        "data\\" + this.packnamespace + "\\functions\\core"
      );
      packcorefuncfolder.file("init.mcfunction", window.atob(INIT_MCFUNC));
      packcorefuncfolder.file(
        "main.mcfunction",
        window
          .atob(MAIN_MCFUNC)
          .replace(/packnamespace/g, this.packnamespace)
          .replace(/discbase/g, this.discbase)
      );
      packcorefuncfolder.file(
        "track.mcfunction",
        window.atob(TRACK_MCFUNC).replace(/packnamespace/g, this.packnamespace)
      );
      // FILE: DatPack/data/<packnamespace>/functions/play.mcfunction =============
      // FILE: DatPack/data/<packnamespace>/functions/get_all.mcfunction ==========
      // FILE: DatPack/data/<packnamespace>/functions/display_lrc.mcfunction ======
      var packfuncfolder = zip.folder(
        "data\\" + this.packnamespace + "\\functions"
      );
      var play_cmd = "#Play Music...\n\n";
      var disp_lrc_cmd = "#Display Lyrics...\n\n";
      var get_all_cmd = "#Get all custom records...\n\n";
      for (var rec in this.records) {
        play_cmd += getPlayCmd(
          Number(this.indexstart) + Number(rec),
          this.records[rec].id,
          this.packnamespace,
          this.discbase
        );
        get_all_cmd += getGiveCmd(
          Number(this.indexstart) + Number(rec),
          this.records[rec].artist,
          this.records[rec].name,
          this.discbase,
          "pink"
        );
        if (
          this.records[rec].lyrics &&
          this.records[rec].lyrics != EMPTY_SYMBOL
        )
          disp_lrc_cmd += getDispLrcCmd(
            Number(this.indexstart) + Number(rec),
            this.records[rec].id,
            this.packnamespace
          );
      ...
      packfuncfolder.file("display_lrc.mcfunction", disp_lrc_cmd);
      packfuncfolder.file("get_all.mcfunction", get_all_cmd);
      var packcustomfuncfolder = zip.folder(
        "data\\" + this.packnamespace + "\\functions\\custom"
      );
      ...
      for (var rec in this.records) {
        packcustomfuncfolder.file(
          this.records[rec].id + ".mcfunction",
          genPlayCommand(
            Number(this.indexstart) + Number(rec),
            this.records[rec].id
          )
        ...
        if (
          ...
          proc(this.records[rec].lyrics);
          const proced = genTitleCommand(
            Number(this.indexstart) + Number(rec),
            this.records[rec].id,
            this.packnamespace,
            this.records[rec].artist,
          ...
          packcustomfuncfolder.file(
            this.records[rec].id + "_lrc.mcfunction",
            proced
          );
        }
      }
      this.pagetitle = "𝔾𝕖𝕥 𝕐𝕠𝕦𝕣 𝔻𝕒𝕥ℙ𝕒𝕔𝕜!";
      zip.generateAsync({ type: "blob" }).then(function (content) {
        saveAs(content, "DiscPlus Dat Pack.zip");
      });
    }
  }
};
